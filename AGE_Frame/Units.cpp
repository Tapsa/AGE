...
string AGE_Frame::GetUnitName(int index, short civ, bool Filter)
{
	...
	if(Filter)
	{
        ...
        for(size_t loop = 0; loop < filters; ++loop)
        {
            ...
            else if(label.compare(Type20[12]) == 0) Name += "SR "+FormatInt(UnitPointer->CollisionSize.x)+" "+FormatInt(UnitPointer->CollisionSize.y);
            else if(label.compare(Type20[13]) == 0) Name += "B1 "+FormatFloat(UnitPointer->CollisionSize.z);
            ...
            else if(label.compare(Type20[23]) == 0) Name += "PBT "+FormatInt(UnitPointer->PlacementSideTerrain.first)+" "+FormatInt(UnitPointer->PlacementSideTerrain.second);
            ...
            else if(label.compare(Type20[25]) == 0) Name += "ER "+FormatInt(UnitPointer->ClearanceSize.first)+" "+FormatInt(UnitPointer->ClearanceSize.second);
            ...
            else if(label.compare(Type20[32]) == 0) Name += "BA "+FormatInt(UnitPointer->BlastDefenseLevel);
            else if(label.compare(Type20[33]) == 0) Name += "U2 "+FormatInt(UnitPointer->SubType);
            ...
            else if(label.compare(Type20[44]) == 0) Name += "U7 "+FormatInt(UnitPointer->Unknown7);
            ...
            else if(label.compare(Type20[54]) == 0) Name += "SS "+FormatInt(UnitPointer->SelectionShapeSize.x)+" "+FormatInt(UnitPointer->SelectionShapeSize.y);
            else if(label.compare(Type20[55]) == 0) Name += "B2 "+FormatFloat(UnitPointer->SelectionShapeSize.z);
...
void AGE_Frame::OnUnitsInsert(wxCommandEvent &event)
{
	...
	for(size_t loop = 0; loop < dataset->Civs.size(); ++loop)
	{
		...
		if(EnableIDFix)
		for(size_t loop2 = UnitIDs[0];loop2 < dataset->Civs[0].Units.size(); ++loop2) // ID Fix
		{
...
void AGE_Frame::OnUnitsDelete(wxCommandEvent &event)
{
	...
	for(short civ = 0; civ < dataset->Civs.size(); ++civ)
	{
		...
		if(EnableIDFix)
		for(size_t loop = UnitIDs[0]; loop < dataset->Civs[0].Units.size(); ++loop) // ID Fix
		{
...
void AGE_Frame::OnUnitsPaste(wxCommandEvent &event)
{
	...
	if(Paste11)
	{
		if(Paste11Check(UnitIDs.size(), copies.Dat.UnitExists[0].size()))
		{
			...
			for(short civ = 0; civ < dataset->Civs.size(); ++civ)
			{
				for(size_t loop = 0; loop < copies.Dat.UnitCopies[0].size(); ++loop)
				{
...
void AGE_Frame::OnUnitsPasteInsert(wxCommandEvent &event)
{
	...
	for(short civ = 0; civ < dataset->Civs.size(); ++civ)
	{
		...
		if(EnableIDFix) // ID Fix
		for(size_t loop = UnitIDs[0];loop < dataset->Civs[0].Units.size(); ++loop)
		{
			dataset->Civs[civ].Units[loop].ID1 = loop;
			dataset->Civs[civ].Units[loop].ID2 = loop;
			if(GenieVersion >= genie::GV_AoK)
...
void AGE_Frame::PasteUnits(bool OneOnOne)
{
	...
	if(copies.Dat.AllCivs & 0x01) // Paste from all civs to all civs.
	{
		...
		for(short civ = 0; civ < CivCount; ++civ)
		{
			for(size_t loop = 0; loop < copies.Dat.UnitCopies[0].size(); ++loop) // selections
			{
...
void AGE_Frame::ListUnitDamageGraphics()
{
	...
	&& dataset->Civs[UnitCivID].UnitPointers[UnitIDs[0]] != 0)
	{
		...
		for(size_t loop = 0; loop < dataset->Civs[UnitCivID].Units[UnitIDs[0]].DamageGraphics.size(); ++loop)
		{
...
void AGE_Frame::ListUnitAttacks()
{
	...
	&& dataset->Civs[UnitCivID].Units[UnitIDs[0]].Type <= 80)
	{
		...
		for(size_t loop = 0; loop < dataset->Civs[UnitCivID].Units[UnitIDs[0]].Type50.Attacks.size(); ++loop)
		{
...
void AGE_Frame::ListUnitArmors()
{
	...
	&& dataset->Civs[UnitCivID].Units[UnitIDs[0]].Type <= 80)
	{
		...
		for(size_t loop = 0; loop < dataset->Civs[UnitCivID].Units[UnitIDs[0]].Type50.Armours.size(); ++loop)
		{
...
{
	short CommandType = (GenieVersion >= genie::GV_AoK) ? dataset->UnitHeaders[UnitIDs[0]].Commands[index].Type : dataset->Civs[UnitCivID].Units[UnitIDs[0]].Bird.Commands[index].Type;
	switch(CommandType)
	{
...
void AGE_Frame::ListUnitCommands()
{
	...
	{
        if(Units_ListV->GetSelectedItemCount())
		for(size_t loop = 0; loop < dataset->UnitHeaders[UnitIDs[0]].Commands.size(); ++loop)
		{
	...
	else	// AoE or RoR
	{
		...
		&& dataset->Civs[UnitCivID].Units[UnitIDs[0]].Type <= 80)
		{
			...
			for(size_t loop = 0; loop < dataset->Civs[UnitCivID].Units[UnitIDs[0]].Bird.Commands.size(); ++loop)
			{
				wxString Name = " "+FormatInt(loop)+" - "+GetUnitCommandName(loop);
	...
	if(selections > 0)
	{
		...
		for(auto sel = selections; sel--> 0;)
		{
			for(short vecCiv = (GenieVersion < genie::GV_AoK) ? SelectedCivs.size() : 1; vecCiv--> 0;)
			{
				...
				UnitCommands_Graphics[0]->prepend(&CommandPointer->ToolGraphicID);
...
void AGE_Frame::OnUnitCommandsInsert(wxCommandEvent &event)
{
	...
	if(GenieVersion >= genie::GV_AoK)
	{
		...
		if(EnableIDFix)
		for(size_t loop2 = CommandIDs[0];loop2 < dataset->UnitHeaders[UnitIDs[0]].Commands.size(); ++loop2) // ID Fix
	...
	else
	{
		for(size_t loop = 0; loop < dataset->Civs.size(); ++loop)
		{
			if(dataset->Civs[loop].UnitPointers[UnitIDs[0]] != 0)
			{
				...
				if(EnableIDFix)
				for(size_t loop2 = CommandIDs[0];loop2 < dataset->Civs[0].Units[UnitIDs[0]].Bird.Commands.size(); ++loop2) // ID Fix
...
//	Type 10+
	...
	Units_ClearanceSize_Grid = new wxGridSizer(2, 0, 0);
...
//	Type 10+
	...
	Units_SizeRadius_Text = new wxStaticText(Units_Scroller, wxID_ANY, " Collision Size XY", wxDefaultPosition, wxSize(-1, 15), wxALIGN_LEFT | wxST_NO_AUTORESIZE);
	...
	Units_ClearanceSize_Text = new wxStaticText(Units_Scroller, wxID_ANY, " Clearance Size XY", wxDefaultPosition, wxSize(-1, 15), wxALIGN_LEFT | wxST_NO_AUTORESIZE);
	...
	Units_BlastDefenseLevel_Text = new wxStaticText(Units_Scroller, wxID_ANY, " Blast Defense Level *", wxDefaultPosition, wxSize(-1, 15), wxALIGN_LEFT | wxST_NO_AUTORESIZE);
	Units_SubType_Text = new wxStaticText(Units_Scroller, wxID_ANY, " Sub Type *", wxDefaultPosition, wxSize(-1, 15), wxALIGN_LEFT | wxST_NO_AUTORESIZE);
	...
	Units_HPBarHeight2_Text = new wxStaticText(Units_Scroller, wxID_ANY, " Selection Shape Size Z *", wxDefaultPosition, wxSize(-1, 15), wxALIGN_LEFT | wxST_NO_AUTORESIZE);
	...
	Units_Unknown10_Text = new wxStaticText(Units_Scroller, wxID_ANY, " Edible Meat? *", wxDefaultPosition, wxSize(-1, 15), wxALIGN_LEFT | wxST_NO_AUTORESIZE);
...
//	Type 40+
	...
	Units_TaskSwapID_Text = new wxStaticText(Units_Scroller, wxID_ANY, " Task Swap ID *", wxDefaultPosition, wxSize(-1, 15), wxALIGN_LEFT | wxST_NO_AUTORESIZE);
...
//	Type 50+
	...
	Units_BlastWidth_Text = new wxStaticText(Units_Scroller, wxID_ANY, " Blast Width *", wxDefaultPosition, wxSize(-1, 15), wxALIGN_LEFT | wxST_NO_AUTORESIZE);
...
//	Type 60 only
	...
	Units_SmartMode_Text = new wxStaticText(Units_Scroller, wxID_ANY, " Smart Mode *", wxDefaultPosition, wxSize(-1, 15), wxALIGN_LEFT | wxST_NO_AUTORESIZE);
...
//	Type 80
	...
	Units_Unknown35_Text = new wxStaticText(Units_Scroller, wxID_ANY, " Unknown 35 *", wxDefaultPosition, wxSize(-1, 15), wxALIGN_LEFT | wxST_NO_AUTORESIZE);
...
//	Data Containers
	...
	Units_ID1 = AGETextCtrl::init(CShort, &uiGroupUnit, this, &popUp, Tab_Units, true);
	Units_ID2 = AGETextCtrl::init(CShort, &uiGroupUnit, this, &popUp, Tab_Units, true);
	Units_ID3 = AGETextCtrl::init(CShort, &uiGroupUnit, this, &popUp, Tab_Units, true);
	...
	DamageGraphics_ApplyMode_Text = new wxStaticText(Units_Scroller, wxID_ANY, " Old Unknown Mode *", wxDefaultPosition, wxSize(-1, 15), wxALIGN_LEFT | wxST_NO_AUTORESIZE);
	...
	DamageGraphics_Unknown2_Text = new wxStaticText(Units_Scroller, wxID_ANY, " Unknown Mode *", wxDefaultPosition, wxSize(-1, 15), wxALIGN_LEFT | wxST_NO_AUTORESIZE);
	...
	Units_IconID = AGETextCtrl::init(CShort, &uiGroupUnit, this, &popUp, Units_Scroller, true);
	...
	Units_IconAngle = AGETextCtrl::init(CShort, &uiGroupUnit, this, &popUp, Units_Scroller, true);
	...
	Units_Enabled = AGETextCtrl::init(CByte, &uiGroupUnit, this, &popUp, Units_Scroller);
	...
	Units_Disabled = AGETextCtrl::init(CByte, &uiGroupUnit, this, &popUp, Units_Scroller);
	...
	Units_AirMode = AGETextCtrl::init(CByte, &uiGroupUnit, this, &popUp, Units_Scroller);
	...
	Units_FlyMode = AGETextCtrl::init(CByte, &uiGroupUnit, this, &popUp, Units_Scroller);
	...
	Units_AnimalMode = AGETextCtrl::init(CByte, &uiGroupUnit, this, &popUp, Units_Scroller);
	...
	Units_AdjacentMode = AGETextCtrl::init(CByte, &uiGroupUnit, this, &popUp, Units_Scroller);
	...
	Units_DisappearsWhenBuilt = AGETextCtrl::init(CByte, &uiGroupUnit, this, &popUp, Units_Scroller);
	...
	Units_HeroMode = AGETextCtrl::init(CByte, &uiGroupUnit, this, &popUp, Units_Scroller);
    ...
    Units_ActionWhenDiscoveredID_Text = new wxStaticText(Units_Scroller, wxID_ANY, " Action When Discovered *", wxDefaultPosition, wxSize(-1, 15), wxALIGN_LEFT | wxST_NO_AUTORESIZE);
    Units_ActionWhenDiscoveredID = AGETextCtrl::init(CShort, &uiGroupUnit, this, &popUp, Units_Scroller, true);
	...
	for(size_t loop = 0; loop < 2; ++loop)
	Units_SizeRadius[loop] = AGETextCtrl::init(CFloat, &uiGroupUnit, this, &popUp, Units_Scroller, true);
	for(size_t loop = 0; loop < 2; ++loop)
	Units_ClearanceSize[loop] = AGETextCtrl::init(CFloat, &uiGroupUnit, this, &popUp, Units_Scroller, true);
	Units_SelectionRadiusBox = new wxBoxSizer(wxHORIZONTAL);
	for(size_t loop = 0; loop < 2; ++loop)
	Units_SelectionRadius[loop] = AGETextCtrl::init(CFloat, &uiGroupUnit, this, &popUp, Units_Scroller, true);
	...
	for(size_t loop = 0; loop < 4; ++loop)
	{
		...
		Units_AnnexUnitMisplacement[loop][loop2] = AGETextCtrl::init(CFloat, &uiGroupUnit, this, &popUp, Units_Scroller);
	}
	for(size_t loop = 0; loop < Units_LootSwitch.size(); ++loop)
	Units_LootSwitch[loop] = AGETextCtrl::init(CByte, &uiGroupUnit, this, &popUp, Units_Scroller);
	...
	UnitCommands_One_Text = new wxStaticText(Units_Scroller, wxID_ANY, "Enabled ", wxDefaultPosition, wxSize(-1, 15), wxALIGN_LEFT | wxST_NO_AUTORESIZE);
...
//	Type 10+
	...
	Units_PlacementSideTerrain_Holder->Add(Units_PlacementSideTerrain_Text, 0, wxEXPAND);
	Units_PlacementTerrain_Holder->Add(Units_PlacementTerrain_Text, 0, wxEXPAND);
	Units_ClearanceSize_Holder->Add(Units_ClearanceSize_Text, 0, wxEXPAND);
	...
	Units_BlastDefenseLevel_Holder->Add(Units_BlastDefenseLevel_Text, 0, wxEXPAND);
	Units_SubType_Holder->Add(Units_SubType_Text, 0, wxEXPAND);
	...
	Units_Unknown10_Holder->Add(Units_Unknown10_Text, 0, wxEXPAND);
...
//	Type 40+
	...
	Units_TaskSwapID_Holder->Add(Units_TaskSwapID_Text, 0, wxEXPAND);
...
//	Type 50+
	...
	Units_BlastWidth_Holder->Add(Units_BlastWidth_Text, 0, wxEXPAND);
	...
	Units_AccuracyDispersion_Holder->Add(Units_AccuracyDispersion_Text, 0, wxEXPAND);
...
//	Type 60 only
	...
	Units_SmartMode_Holder->Add(Units_SmartMode_Text, 0, wxEXPAND);
...
//	Type 70+
	...
	Units_UnknownType_Holder->Add(Units_UnknownType_Text, 0, wxEXPAND);
...
//	Type 10+
	...
	for(size_t loop = 0; loop < 2; ++loop)
	Units_PlacementSideTerrainGrid_Holder->Add(Units_PlacementSideTerrain[loop], 1, wxEXPAND);
	for(size_t loop = 0; loop < 2; ++loop)
	Units_PlacementSideTerrainGrid_Holder->Add(Units_PlacementSideTerrain_ComboBox[loop], 1, wxEXPAND);
	...
	for(size_t loop = 0; loop < 2; ++loop)
	Units_ClearanceSize_Grid->Add(Units_ClearanceSize[loop], 1, wxEXPAND);
	Units_ClearanceSize_Holder->Add(Units_ClearanceSize_Grid, 1, wxEXPAND);
	...
	Units_BlastDefenseLevel_Holder->Add(Units_BlastDefenseLevel, 1, wxEXPAND);
	Units_SubType_Holder->Add(Units_SubType, 1, wxEXPAND);
...
//	Type 40+
	...
	Units_ActionWhenDiscoveredID_Holder->Add(Units_ActionWhenDiscoveredID_Text, 0, wxEXPAND);
	...
	Units_TaskSwapID_Holder->Add(Units_TaskSwapID, 1, wxEXPAND);
...
//	Type 60 only
	...
	Units_SmartMode_Holder->Add(Units_SmartMode, 1, wxEXPAND);
...
//	Type 70+
	...
	Units_CostAmount_Holder->Add(Units_CostAmount[1], 4, wxEXPAND | wxLEFT, 5);
	Units_CostAmount_Holder->Add(Units_CostAmount[2], 4, wxEXPAND | wxLEFT, 5);
	Units_CostUsed_Holder->Add(Units_CostUsed[0], 3, wxEXPAND);
	Units_CostUsed_Holder->Add(Units_CostUsed_CheckBox[0], 1, wxEXPAND | wxLEFT, 2);
	Units_CostUsed_Holder->Add(Units_CostUsed[1], 3, wxEXPAND | wxLEFT, 5);
	Units_CostUsed_Holder->Add(Units_CostUsed_CheckBox[1], 1, wxEXPAND | wxLEFT, 2);
	Units_CostUsed_Holder->Add(Units_CostUsed[2], 3, wxEXPAND | wxLEFT, 5);
	Units_CostUsed_Holder->Add(Units_CostUsed_CheckBox[2], 1, wxEXPAND | wxLEFT, 2);
...
//	Type 80
	...
	Units_ResourceStorage_Holder[1]->Add(ResourceStorage_Amount[1], 4, wxEXPAND | wxLEFT, 5);
	Units_ResourceStorage_Holder[1]->Add(ResourceStorage_Amount[2], 4, wxEXPAND | wxLEFT, 5);
	...
	Units_ResourceStorage_Holder[2]->Add(ResourceStorage_Enabled[1], 4, wxEXPAND | wxLEFT, 5);
	Units_ResourceStorage_Holder[2]->Add(ResourceStorage_Enabled[2], 4, wxEXPAND | wxLEFT, 5);
    ...
    sizer_slp->Add(slp_snow, 0, wxEXPAND | wxLEFT, 5);
    sizer_slp->Add(slp_garrison, 0, wxEXPAND | wxLEFT, 5);
	...
	Units_Attacks_Grid_Data2->Add(Units_AccuracyDispersion_Holder, 1, wxEXPAND);
	...
	Units_Armors_DataArea->Add(Armors_Class_Holder, 0, wxEXPAND | wxTOP, 5);
	Units_Armors_Holder_Data3->Add(Units_DefaultArmor_Holder, 0, wxEXPAND | wxTOP, 5);
	Units_Armors_Holder_Data3->Add(Units_TerRestrictionForDmgMultiply_Holder, 0, wxEXPAND | wxTOP, 5);
	Units_Armors_Holder_Data3->Add(Units_DisplayedMeleeArmour_Holder, 0, wxEXPAND | wxTOP, 5);
	Units_Armors_Holder_Data3->Add(Units_DisplayedPierceArmour_Holder, 0, wxEXPAND | wxTOP, 5);
	Units_Armors_Holder_Data3->Add(Units_BlastDefenseLevel_Holder, 0, wxEXPAND | wxTOP, 5);
	...
	Units_Attacks->Add(Units_Attacks_DataArea, 1, wxEXPAND | wxLEFT, 5);
	Units_Armors->Add(Units_Armors_ListArea, 1, wxEXPAND);
	Units_Armors->Add(Units_Armors_DataArea, 1, wxEXPAND | wxLEFT, 5);
	Units_Attacks_Holder->Add(Units_Attacks, 1, wxEXPAND);
	Units_Attacks_Holder->Add(Units_Attacks_Holder_Data, 1, wxEXPAND | wxLEFT | wxTOP, 5);
	...
	Units_ProjectilesArea1_Grid->Add(Units_SmartMode_Holder, 1, wxEXPAND);
	Units_ProjectilesArea1_Grid->Add(Units_DropAnimationMode_Holder, 1, wxEXPAND);
	Units_ProjectilesArea1_Grid->Add(0, 0);
	Units_ProjectilesArea1_Grid->Add(0, 0);
	...
	Units_AttributesModes1_Grid->Add(Units_SubType_Holder, 1, wxEXPAND);
	...
	Units_Attributes4_Grid->Add(Units_ActionWhenDiscoveredID_Holder, 1, wxEXPAND | wxRIGHT, 5);
	...
	Units_AttributesSizes_Holder->Add(Units_ClearanceSize_Holder, 1, wxEXPAND);
	...
	Units_AttributesSelection1_Grid->Add(0, 0);
	...
	Units_ResourceStorageHeader_Holder->Add(Units_ResourceStorage_Holder[1], 0, wxEXPAND | wxTOP, 5);
	...
	Units_CostHeader_Holder->Add(Units_CostAmount_Holder, 0, wxEXPAND | wxTOP, 5);
	Units_CostHeader_Holder->Add(Units_CostUsed_Holder, 0, wxEXPAND | wxTOP, 5);
	...
	Units_Attributes_Holder->Add(Units_AttributesModes1_Grid, 0, wxEXPAND | wxTOP, 5);
	Units_Attributes_Holder->Add(Units_Attributes2_Grid, 0, wxEXPAND | wxTOP, 5);
	...
	Units_Attributes_Holder->Add(Units_HPBars_Grid, 0, wxEXPAND | wxTOP, 5);
	...
	Units_Attributes_Holder->Add(Units_ResourceStorageHeader_Holder, 0, wxEXPAND | wxTOP, 5);
	Units_Attributes_Holder->Add(Units_CostHeader_Holder, 0, wxEXPAND | wxTOP, 5);
	Units_Attributes_Holder->Add(Units_Attributes3_Grid, 0, wxEXPAND | wxTOP, 5);
	Units_Attributes_Holder->Add(Units_AnnexUnit1_Holder, 0, wxEXPAND | wxTOP, 5);
	Units_Attributes_Holder->Add(Units_AnnexUnitMisplacement1_Holder, 0, wxEXPAND | wxTOP, 5);
	Units_Attributes_Holder->Add(Units_LootSwitch_Holder, 0, wxEXPAND | wxTOP, 5);
	...
	Units_TrainSound3_Holder->Add(Units_TrainSound2_Holder, 1, wxEXPAND | wxLEFT, 5);
	...
	Units_SoundsArea1_Holder->Add(Units_DyingSound_Holder, 1, wxEXPAND | wxLEFT, 5);
	Units_SoundsArea1_Holder->Add(Units_TrainSound_Holder, 2, wxEXPAND | wxLEFT, 5);
	...
	Units_CommandHolder_Lists->Add(Units_UnitCommands_ListV, 1, wxEXPAND | wxBOTTOM | wxTOP, 2);
	...
	UnitCommands_1_Holder->Add(UnitCommands_ID_Holder, 1, wxEXPAND | wxLEFT, 5);
	...
	Units_CommandHolder_Data2->Add(UnitCommands_Type_Holder, 0, wxEXPAND | wxTOP, 5);
	Units_CommandHolder_Data2->Add(Units_CommandHolder_Grid1, 0, wxEXPAND | wxTOP, 5);
	Units_CommandHolder_Data2->Add(Units_CommandHolder_Grid2, 0, wxEXPAND | wxTOP, 5);
	...
	Units_CommandHolder_Data1->Add(Units_CommandHolder_Grid3, 1, wxEXPAND | wxRESERVE_SPACE_EVEN_IF_HIDDEN | wxLEFT, 5);

	Units_CommandHolder_Data->Add(Units_CommandHolder_Data1, 0, wxEXPAND);
	Units_CommandHolder_Data->Add(UnitCommands_Graphics_Grid, 0, wxEXPAND | wxTOP, 5);
